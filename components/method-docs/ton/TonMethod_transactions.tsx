...
export function TonMethod_transactions(props: GenericMethodPropsReplacing) {
  return (
    <TonMethod
      method="transactions"
      network="ton"
...
const CODE_SNIPPETS: Array<CodeSnippetObject> = [
    ...
    code: () => `curl --request GET \\
     --url '\ ${DRPC_ENDPOINT_URL_TON_V3}transactions?limit=128&offset=0&sort=desc' \\
     --header 'accept: application/json'`,
  },
  {
    language: "js",
    code: () => `const url = \`${DRPC_ENDPOINT_URL_TON_V3}transactions?limit=128&offset=0&sort=desc\`;
...
const url = \`${DRPC_ENDPOINT_URL_TON_V3}transactions?limit=128&offset=0&sort=desc\`;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let url = "${DRPC_ENDPOINT_URL_TON_V3}transactions?limit=128&offset=0&sort=desc";
