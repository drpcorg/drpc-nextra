...
export function TonMethod_transactionsByMasterchainBlock (props: GenericMethodPropsReplacing) {
  return (
    <TonMethod
      method="transactionsByMasterchainBlock"
      network="ton"
      cu = {100}
      description={
          "Retrieves all transactions contained within a specified masterchain block"}
      useCases={USE_CASES}
...
const CODE_SNIPPETS: Array<CodeSnippetObject> = [
    ...
    code: () => `curl --request GET \\
     --url '\ ${DRPC_ENDPOINT_URL_TON_V3}transactionsByMasterchainBlock?seqno=40467413&limit=128&offset=0&sort=desc' \\
     --header 'accept: application/json'`,
  },
  {
    language: "js",
    code: () => `const url = \`${DRPC_ENDPOINT_URL_TON_V3}transactionsByMasterchainBlock?seqno=40467413&limit=128&offset=0&sort=desc\`;
...
const url = \`${DRPC_ENDPOINT_URL_TON_V3}transactionsByMasterchainBlock?seqno=40467413&limit=128&offset=0&sort=desc\`;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let url = "${DRPC_ENDPOINT_URL_TON_V3}transactionsByMasterchainBlock?seqno=40467413&limit=128&offset=0&sort=desc";
...
const USE_CASES = [
  "Retrieve transactions in a masterchain block",
  "Analyze transactions by block",
  "Audit block-specific transaction data",
];
