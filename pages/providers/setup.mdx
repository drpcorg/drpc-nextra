...
Note: We expect you to set up your Dshackle with TLS support.
...
tls:
  enabled: true
  server:
    certificate: "/path_to_ssl_certs/127.0.0.1.crt"
    key: "/path_to_ssl_certs/127.0.0.1.p8.key"
...
auth:
  ...
  server:
    keys:
      provider-private-key: /path_to_keys/private-my-test-key.p8.key
...
If you have set everything up and want to check if Dshackle can be reached from outside and is correctly configured, you can use dshackle-cli. Just follow the [instructions](https://github.com/p2p-org/dshackle/blob/master/dshackle-cli/README.md).

## Mutual Authorization

Previously we used TLS for mutual authorization as well - you provided us your CA and got ours. This ensured that we were us and you were you. However, the process of manual generating certificates could be tricky and cause some further issues if something is configured incorrectly. That's why we've decided to revise our mutual authorization part.

Our new authorization algorithm is based on private/public key pairs and JWT tokens. We have our private key and your public key, and you have your own private key and our public key. Using these keys, we generate JWT tokens on both sides, verify them, and if they are valid, authorization has been passed.

Currently we support only RSA keys. You can use the following tiny script to generate such pair, specifying the first argument (key suffix) `rsa-key-generator.sh`:
...
For example, we want to generate our pair with `my-test-key` suffix, so we execute `./rsa-key-generator.sh my-test-key`. After that we will get a pair of our keys, private `private-my-test-key.p8.key` and public `public-my-test-key.pem`. Share your **public key** (and only **public**) with us.
...
`external-public-key` is our key `public-drpc.pem`:
...
**Attention**: using this authorization doesn't mean that you can just remove your TLS part from config doing nothing. Your connection must be secure in any case. But with this you can use your own certificates (e.g. let's encrypt) or remove tls config and use TLS termination inside your network.

For example, you can add the following configuration for TLS with your own certificate and key

```yaml
tls:
  enabled: true
  server:
    certificate: /path_to_ssl_certs/your_cert_name.crt
    key: /path_to_ssl_certs/your_key_name.p8.key
```
