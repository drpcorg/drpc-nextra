...
<Callout type="warning">
  Quorum requests in NodeCore bypass the response cache and integrity layer,
  are always buffered (streaming is disabled), and work over HTTP(S) only —
  gRPC, WebSocket and sticky-send methods like `eth_sendRawTransaction` are
  rejected with a `quorum not supported` error.
</Callout>
