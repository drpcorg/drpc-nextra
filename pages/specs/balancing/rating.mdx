...
It is important to keep multipliers steadily increasing with 2^30 (or 2^29) be at the very end. The reason is that 1/2^30 < 1e-9, and 1e-9 is the minimal possible rating. It means that any rating less than 1e-9 will be scaled to 1e-9, and the information about the relationship between ratings less than 1e-9 will be completely lost (all ratings will be artificially equal).

Once set, a simple equation is constructed:

`x + x/m_1 + x/m_2 + ... + x/m_n = 1` 

Where `x` — pre-rating of the fastest provider, and `m_i` — multipliers chosen according to the mapping.
