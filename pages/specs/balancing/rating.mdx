...
The list of incentives is updated. We may introduce new incentives or end old ones. A full list of inventives is available for providers in their account.

The rating algorithm takes the prediction, the prices, mathcing incentives, and produce new ratings every 5 seconds. The ratings are calculated and stored on each dproxy independently, meaning the ratings could vary but not substantially.
...
Incentives match features are transofrmed in a number between 0 and 1 for each provider in a similar way. Usually, 1 means that this provider is the only one matching the incentive rule, and 0 means that the provider does not match the incentive conditions.

Then, incentive and price features are summed up together with a vector of ones. The produces a vector of features where each feature is more or equal to 1.
...
The **latency** and **error rates** are still the most important providers.
...
The rating can be improved or downgraded only by 4 factors: latency, errors, price and matching incentive conditions. 
